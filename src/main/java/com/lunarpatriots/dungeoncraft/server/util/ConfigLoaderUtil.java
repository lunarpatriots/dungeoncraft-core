...
public class ConfigLoaderUtil {
  ...
  public static ServerConfig loadServerConfigs() {
    if (serverConfig == null) {
      final Path configPath = FabricLoader.getInstance().getConfigDir().resolve("dungeoncraft-server.yml");
      ...
      if (Files.exists(configPath)) {
        ...
        try {
          ...
          try (InputStream inputStream = ConfigLoaderUtil.class.getClassLoader()
            .getResourceAsStream("dungeoncraft-server.yml")) {
            if (inputStream != null) {
